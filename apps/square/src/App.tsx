...
import { CreatorTaskFeedRoute } from "./routes/CreatorTaskFeedRoute.js";
...
import { useCreatorWalletMetrics } from "./hooks/useCreatorWalletMetrics.js";
...
function RoutedApp({
  ...
  const headerActions = (
    <div className="flex items-center gap-2">
      {loading && <span className="text-sm text-gray-500">正在同步角色视图...</span>}
      {statusMessage && <span className="text-sm text-red-500">{statusMessage}</span>}
    </div>
  );
  ...
  const CreatorWalletRoute = () => {
    const { cards, errorMessage } = useCreatorWalletMetrics();
